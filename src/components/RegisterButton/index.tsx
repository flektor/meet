...
const Button = (
  ...
  return (
    <>
      ...
      <Svg
        className={`mt-2 select-none cursor-pointer transition ${style} ${className}`}
        viewBox="0 0 40 40"
        onClick={onClick}
      >
        <path
          transform="scale(0.85) translate(0,4)"
...
const RegisterButton = (
  ...
  return (
    <>
      ...
      />
      {isRegistered && (
        <DotsLoader className="fill-[#33BBFF] max-h-3 relative top-3.5 right-8 -mr-8 pr-1 z-0" />
      )}
    </>
  );
};
